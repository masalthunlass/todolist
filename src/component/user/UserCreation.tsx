...
const UserCreation: React.FC<props> = ({ onSubmit, editableUser }) => {
    ...
    const [user, setUser] = React.useState(editableUser || { id: '', lastname: '', firstname: '', language: 'none' });
    ...
    return (<div><form onSubmit={onSubmitForm(user, onSubmit)} >
        ...
        <button type="submit" value="ok"> {editableUser ? 'modifier' : 'ok'} </button>
    </form></div>);
}
...
const onSubmitForm = (user, onSubmit) => (event) => {
    ...
    if (onSubmit) {
        onSubmit({ user: { ...user, id: nextIndex() } });
